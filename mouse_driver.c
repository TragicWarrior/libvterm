...
/*
    this var has to exist in the global namespace so we'll tack on the
    date to miminize risk of collission.
*/
#define MSTATE_TRACKER  mouse_state_##__DATE__

mouse_state_t   *MSTATE_TRACKER  = NULL;

void
mouse_driver_init(vterm_t *vterm)
{
    extern mouse_state_t    *MSTATE_TRACKER;
    ...
    */
    if(MSTATE_TRACKER == NULL)
    {
        MSTATE_TRACKER = calloc(1, sizeof(mouse_state_t));

        if(has_mouse() == TRUE)
            MSTATE_TRACKER->origin = NULL;
        else
        {
            MSTATE_TRACKER->origin = vterm;
...
mouse_driver_save_state(vterm_t *vterm)
{
    ...
    */
    if(MSTATE_TRACKER->origin != vterm)
    {
